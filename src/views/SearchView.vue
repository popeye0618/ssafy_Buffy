...
<template><div class="page fade">
  <RouterLink class="btn mb-5" to="/">← 뒤로</RouterLink>
  <h1 class="text-4xl font-extrabold">통합 검색</h1>
  <p class="mt-3 text-[var(--sub)]">장소별 게시판과 커뮤니티 게시글을 한 번에 찾아보세요.</p>
  <form class="mt-7 flex max-w-3xl gap-2" @submit.prevent="search(true)"><label class="relative flex-1"><span class="sr-only">검색어</span><Search class="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--sub)]" :size="19"/><input v-model="input" class="input !pl-12" maxlength="200" placeholder="장소, 축제, 게시글 검색"></label><button class="btn btn-primary" :disabled="loading||!input.trim()">검색</button></form>
  ...
  <div v-else-if="error" class="empty mt-8"><AlertTriangle class="mx-auto text-[var(--danger)]"/><h2 class="mt-3 text-lg font-bold">검색 결과를 불러오지 못했습니다</h2><p class="mt-2 text-[var(--sub)]">{{error}}</p><button class="btn mt-4" @click="search()">다시 시도</button></div>
  <div v-else-if="searched&&!items.length" class="empty mt-8"><Search class="mx-auto text-[var(--sub)]"/><h2 class="mt-3 text-lg font-bold">검색 결과가 없습니다</h2><p class="mt-2 text-[var(--sub)]">다른 검색어로 다시 찾아보세요.</p></div>
  ...
  <div v-else class="empty mt-8"><Search class="mx-auto text-[var(--sub)]"/><h2 class="mt-3 text-lg font-bold">부산의 장소와 이야기를 검색해 보세요</h2></div>
</div></template>
