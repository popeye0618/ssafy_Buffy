...
import { AlertTriangle, CalendarDays, Download, Heart, MapPin, MessageCircle, MessagesSquare, Search, Wifi, WifiOff } from '@lucide/vue'
...
const heroPlaceholder=computed(()=>isMobile.value?app.t('장소, 축제, 게시글 검색','Search Busan'):app.t('장소, 축제, 게시글 검색','Search places, festivals, and posts'))
...
<template><div class="page fade">
  <section class="pt-2 md:pt-3"><div class="grid items-center gap-9 md:grid-cols-[1.08fr_.92fr]"><div class="min-w-0">
    ...
    <form class="mt-7 flex max-w-xl items-center gap-2 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-2 pl-4 shadow-[var(--shadow-md)]" @submit.prevent="goSearch"><Search :size="20"/><input v-model="heroQuery" class="min-w-0 flex-1 bg-transparent p-2 outline-none" :placeholder="heroPlaceholder"><button class="btn btn-primary" :disabled="!heroQuery.trim()">{{app.t('검색','Search')}}</button></form>
  ...
  <section class="section grid gap-5 md:grid-cols-[1.4fr_1fr]"><div><h2 class="section-title mb-4">{{app.t('인기 게시글','Popular posts')}}</h2><div v-if="app.state.popular==='loading'" class="card h-48 animate-pulse bg-[var(--surface)]"></div><div v-else-if="app.state.popular==='error'" class="card p-6"><p class="text-sm text-[var(--danger)]">{{app.errors.popular}}</p><button class="btn mt-4" @click="app.loadPopularPosts(1,5).catch(()=>{})">{{app.t('다시 시도','Try again')}}</button></div><div v-else-if="!app.popularPosts.length" class="card p-6"><div class="flex items-center gap-2 font-bold text-[var(--sub)]"><Heart :size="18"/><span>{{app.t('아직 인기 게시글이 없습니다','No popular posts yet')}}</span></div><p class="mt-3 text-sm leading-6 text-[var(--sub)]">{{app.t('첫 번째 여행 이야기와 추천을 남겨보세요.','Share the first travel story and recommendation.')}}</p></div><div v-else class="card overflow-hidden"><RouterLink v-for="post in app.popularPosts" :key="post.id" :to="`/boards/${post.boardId}/posts/${post.id}`" class="block border-b border-[var(--border)] p-4 last:border-0 hover:bg-[var(--highlight)]"><h3 class="font-bold">{{app.postTitle(post)}}</h3><div class="mt-2 flex gap-4 text-xs text-[var(--sub)]"><span class="metric text-red-500"><Heart :size="14"/>{{post.likes}}</span><span class="metric"><MessageCircle :size="14"/>{{post.comments}}</span></div></RouterLink></div></div>
