...
<template>
  <div :data-theme="app.theme" :style="{ fontSize: `${16 * app.fontScale}px` }" class="min-h-screen bg-[var(--bg)] text-[var(--text)] transition-colors">
    <a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-100 focus:rounded-lg focus:bg-[var(--primary)] focus:p-3">본문 바로가기</a>
    <AppUtilities/>
    <main id="main-content" tabindex="-1">
      <div v-if="route.path !== '/'" class="page-home-wrap"><RouterLink to="/" class="home-shortcut"><Home :size="18"/><span>홈</span></RouterLink></div>
